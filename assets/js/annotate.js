    ...
    rebind('validate', function(){
      ...
      fetchJson(GENES_URL, 'genes.json').then(function(g){
        ...
        return ORT.InferenceSession.create(MODEL_URL, { executionProviders: eps }).then(function(test){
          var D = 1; // we’ll infer zero tensor shape after genes length fetch above, but we didn’t keep it; do a small 1x1
          var zeros = new ORT.Tensor('float32', new Float32Array(D), [1, D]);
          return test.run(function(){
            var inputs = {};
            inputs[test.inputNames[0]] = zeros;
            return inputs;
          }()).then(function(out){
            var any = out[test.outputNames[0]];
            if (!any) {
              var keys = [];
              for (var k in out) if (Object.prototype.hasOwnProperty.call(out,k)) keys.push(k);
              if (keys.length) any = out[keys[0]];
            }
